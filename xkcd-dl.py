...
def download_image(meta):
      ...
      if (not os.path.isfile(save_path+'/'+str(meta['num'])+'_'+comic_file_name[-1])):
            r = http.request('GET', meta['img'])
            try:
                  f = open(save_path+'/'+str(meta['num'])+'_'+comic_file_name[-1], 'bw')
                  f.write(r.data)
                  f.close()
            except e:
...
def download_meta(number):
      ...
      if (number!=-1):
            ...
            # If we have chosen --latest or all, always query the internet, since we do not know the number for the latest comic.
...
def main():
      ...
      if (args.latest or args.number):
            # if a single request is made - by number or only latest, download 1 image and we are done.
            verbose("Downloading image now.")
            download_image(meta)
      elif (args.all):
            # if multiple requests are made --all, run in a loop. decrementing the meta['num'] till we reach 1
            while (meta['num']>=1):
                  download_image(meta)
